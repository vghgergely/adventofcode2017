test = [[179, 2358, 5197, 867, 163, 4418, 3135, 5049, 187, 166 , 4682 , 5080 , 5541 , 172 , 4294, 1397],
...
def checksummer(lists):
    sums = 0
    secsum = 0
    for e in lists:
        rowint = list(map(int,e))
        for i in range(len(e)):
            for j in range(len(e)):
                if i != j:
                    if rowint[i] % rowint[j] == 0:
                        secsum += rowint[i] / rowint[j]
                    
        sums += max(e)
        sums -= min(e)
    return secsum
    
print (checksummer(test))
